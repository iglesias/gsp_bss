function [truth, model, y] = twograph_coupling_bss_gen_problem(params)
...
if isfield(params, 'numGraphs')
  if params.numGraphs ~= 2
    error('Invalid number of graphs (%d) in experiment coupling two graphs.', params.numGraphs)
  end
  numGraphs = 2;
else
  numGraphs = 2;
...
% Generate second ER graph coupled with the first one,
% ensuring that it has one connected component as well.
i = 2;
while true
...
assert(issymmetric(model.G(1).W))
assert(issymmetric(model.G(1).L))
assert(issymmetric(model.G(2).W))
assert(issymmetric(model.G(2).L))
